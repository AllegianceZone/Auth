...
",
		...
		private bool IsGameEligibleForLogging(DataAccess.Game game)
		{
			...
			// Games must have at least 5 players on each side that have played for more than 5 minutes.
			bool hadEnoughEligiblePlayers = true;
			foreach (DataAccess.GameTeam team in game.GameTeams)
			{
				if (team.GameTeamMembers.Where(p => p.GameTeamMemberDuration / 60 >= 5).Count() < 5)
				{
